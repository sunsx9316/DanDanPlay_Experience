...
import MobileVLCKit
import AVFoundation
typealias MediaImage = UIImage
#else
import AppKit
import VLCKit
typealias MediaImage = NSImage
#endif
...
class MediaThumbnailer: NSObject {
    ...
    private class Task: VLCMediaThumbnailerDelegate {
        ...
        private(set) var snapshot: MediaImage?
        ...
        func mediaThumbnailer(_ mediaThumbnailer: VLCMediaThumbnailer!, didFinishThumbnail thumbnail: CGImage!) {
            let img = MediaImage(cgImage: thumbnail)
            DispatchQueue.main.async {
                self.snapshot = img
                self.isFinish = true
            }
    ...
    func snapshot(at time: Int) -> UIImage? {
