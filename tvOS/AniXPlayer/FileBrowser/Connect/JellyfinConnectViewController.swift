//
//  JellyfinConnectViewController.swift
//  AniXPlayer
//
//  tvOS Jellyfin 连接页面 — 复用 Emby API 兼容层
//
...
class JellyfinConnectViewController: RemoteConnectViewController {
    ...
    private func loginWithInfo(_ info: LoginInfo) {
        ...
        fileManager.connectWithLoginInfo(info) { [weak self] error in
            ...
            DispatchQueue.main.async {
                ...
                if let error = error {
                    ANX.logError(.webDav, "Jellyfin 连接失败 error: %@", error as NSError)
