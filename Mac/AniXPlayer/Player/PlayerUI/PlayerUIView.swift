...
class PlayerUIView: BaseView {
    ...
    private(set) var isControlViewHidden = false {
        didSet {
            delegate?.playerUIView(self, didChangeControlViewState: !isControlViewHidden)
        }
    }
    ...
    private func updateMiniProgressVisibility() {
        miniProgressSlider.isHidden = !miniProgressEnabled || !isControlViewHidden
    }
    ...
    func autoShowControlView(completion: (() -> ())? = nil) {
        DispatchQueue.main.async {
            self.setControlViewsHidden(false, animated: true)

            self.autoHiddenTimer?.invalidate()
            self.autoHiddenTimer = Timer.scheduledTimer(withTimeInterval: self.hiddenTime, block: { timer in
                self.autoHideControlView()
            }, repeats: false)
            self.autoHiddenTimer?.fireDate = Date(timeIntervalSinceNow: self.hiddenTime)
    ...
    /// 统一控制 topView / bottomView 显隐 + mini 进度条联动
    private func setControlViewsHidden(_ hidden: Bool, animated: Bool, completion: (() -> Void)? = nil) {
        guard isControlViewHidden != hidden else {
            completion?()
            return
        }
        isControlViewHidden = hidden
        updateMiniProgressVisibility()
        ...
        if animated {
            NSAnimationContext.runAnimationGroup { ctx in
                ...
                self.topView.animator().alphaValue = hidden ? 0 : 1
                self.bottomView.animator().alphaValue = hidden ? 0 : 1
            } completionHandler: {
                completion?()
            }
        } else {
            self.topView.alphaValue = hidden ? 0 : 1
            self.bottomView.alphaValue = hidden ? 0 : 1
            completion?()
        }
    }
    ...
    override func rightMouseDown(with event: NSEvent) {
        ...
        delegate?.onClickRightMouse(playerUIView: self, at: point)
    }
    ...
    @objc private func onTouchStopButton(_ sender: NSButton) {
        delegate?.onTouchStopButton(playerUIView: self)
    }
