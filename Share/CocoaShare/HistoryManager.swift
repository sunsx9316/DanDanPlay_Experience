...
class HistoryManager {
    ...
    private lazy var watchProgressStoreMap: [String : TimeInterval] = {
        return Self.loadMap(storeKey: watchProgressStoreKey)
    }()

    private lazy var lastWatchDateStoreMap: [String : TimeInterval] = {
        return Self.loadMap(storeKey: lastWatchDateStoreKey)
    }()
    ...
    private func storeWatchProgress() {
        Preferences.shared.store.set(self.watchProgressStoreMap, forKey: watchProgressStoreKey)
    }

    private func storeLastWatchDate() {
        Preferences.shared.store.set(self.lastWatchDateStoreMap, forKey: lastWatchDateStoreKey)
    }

    private static func loadMap(storeKey: String) -> [String: TimeInterval] {
        // Try new format (via Store)
        if let value: [String: TimeInterval] = Preferences.shared.store.value(forKey: storeKey) {
            return value
        }
        // Migration: try old format (NSDictionary stored directly in UserDefaults)
